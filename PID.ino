...
const float RAIO_RODA = 0.0336;

// Distancia das rodas em metros
const float DIST_RODAS = 0.2140;

//Pulsos por revolução do eixo da roda
const float PPR_ENCODER = 330;
...
//Posição 
float pos_atual_x = 0.0;
float pos_atual_y = 0.0;
float pos_theta = 0.0;

//tempo de amostragem (mudar depois)
const float DT = 0.05; 
...
void calcula_odometria() {
    ...
    long delta_ticks_dir = ticks_atuais_dir - ticks_esq;
    ...
    float Delta_rad_esq = (delta_ticks_esq/PPR_ENCODER)*2*PI;
    float Delta_rad_dir = (delta_ticks_dir/PPR_ENCODER)*2*PI;

    float Vel_Ang_Dir = (Delta_rad_dir/DT);
    float Vel_Ang_Esq = (Delta_rad_esq/DT);

    float V_Dir = (Delta_rad_dir*RAIO_RODA);
    float V_Esq = (Delta_rad_esq*RAIO_RODA);
    float Vel_Linear = (V_Dir + V_Esq)/2;

    float VarAng = (Delta_rad_esq - Delta_rad_dir)/DIST_RODAS;
    float Vel_Ang_centro = (VarAng/DT);

    //setpoint arbitrario por enquanto
    float erro_int_esq = setpoint_esq - V_Esq
    float erro_int_dir = setpoint_dir - V_Dir
}
...
void aciona_motores(float PWM_esp, float PWM_dir) {
    if PWM_esquerdo < 0:
        digitalWrite(MOT_ESQ_IN1, HIGH);
        digitalWrite(MOT_ESQ_IN2, LOW);
        analogWrite(MOT_ESQ_PWM, abs(PWM_esquerdo));
    if PWM_esquerdo > 0:
        digitalWrite(MOT_ESQ_IN1, HIGH);
        digitalWrite(MOT_ESQ_IN2, LOW);
        analogWrite(MOT_ESQ_PWM, abs(PWM_esquerdo));
    if PWM_direito < 0:
        digitalWrite(MOT_ESQ_IN1, HIGH);
        digitalWrite(MOT_ESQ_IN2, LOW);
        analogWrite(MOT_ESQ_PWM, abs(PWM_direito));
    if PWM_direito > 0:
        digitalWrite(MOT_ESQ_IN1, HIGH);
        digitalWrite(MOT_ESQ_IN2, LOW);
        analogWrite(MOT_ESQ_PWM, abs(PWM_direito));
...
void loop() {
    ...
    if (tempo_atual - tempo_anterior >= INTERVALO_AMOSTRAGEM_MS) {
        calcula_odometria();
        controle_pid();
        float deltaT = tempo_atual - tempo_anterior

        float RPM_esq = delta_ticks_esq/deltaT

        float RPM_dir = delta_ticks_dir/deltaT

        //setpoint_esq é a velocidade definida pelo ros, para o motor esquerdo que vai ser definido depois de aplicar ros no codigo
        float PWM_esquerdo = controle_pid(setpoint_esq, V_Esq, erro_int_esq, erro_anterior_esq)
        //setpoint_esq é a velocidade definida pelo ros, para o motor direito que vai ser definido depois de aplicar ros no codigo
        float PWM_direito = controle_pid(setpoint_dir, V_Dir, erro_int_dir, erro_anterior_dir)

        aciona_motores(PWM_esq, PWM_dir);
        ...
        erro_anterior_dir = erro_int_dir
        erro_anterior_esq = erro_int_esq
    }
}